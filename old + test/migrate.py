...
try:
    ...
    for item in mongo_data:
        try:
            ...
            image_url = item.get("image")  # Fetch image URL
            price = float(item.get("price", 0))  # Default to 0 if price is missing
            retrieved_on = datetime.now()
            is_active = True  # Default active status

            if not all([store_name, product_name, product_url, image_url]):
            ...
            if product_result is None:
                pg_cursor.execute(
                    """
                    INSERT INTO products (store_id, product_name, product_url, image_url, is_active)
                    VALUES (%s, %s, %s, %s, %s) RETURNING product_id
                    """,
                    (store_id, product_name, product_url, image_url, is_active),
                )
